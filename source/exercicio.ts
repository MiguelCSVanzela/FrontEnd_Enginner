...
const multiplica = (multiplicador: number, multiplicando: number): number => {
...
const finonacci = (termo: number): number[] => {
    let resultado: number[] = [0, 1];
    for (let i: number = 2; i < termo; i++) {
...
let casos: string[] = ["Dormir", "Exercitar", "Comunicar"];
let nome: string = comand(`Olá, Sou Jarvis seu assistente. Como você se chama?  `);
let saudar: string = `Olá ${nome}, ${saudacao()}`;
console.log(saudar);
let option: string = "";

const chat = (option: string): string => {
    let mensagem: string = "";
